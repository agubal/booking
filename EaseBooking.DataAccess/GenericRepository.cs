...
using System.IO;
using System.Linq;
using EaseBooking.DataAccess.Database;
using EaseBooking.Entities.Common;
using Newtonsoft.Json;

namespace EaseBooking.DataAccess
{
    /// <summary>
    ...
    public class GenericRepository<T, TIdentity> : IRepository<T, TIdentity> where T : class, IIdentifier<TIdentity>
    {
        ...
        {
            //Seed fake data. Works only once
            FakeDatabase.InitializeFakeDataBase();
        }
        ...
        public bool Update(T entity)
        {
            ...
            var newEntities = allEntities.Select(item => item.Id.Equals(entity.Id) ? entity : item).ToList();
            string json = JsonConvert.SerializeObject(newEntities);
